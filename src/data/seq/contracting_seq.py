...
class ContractingSequenceDataGenerator(SequenceDataGenerator):
    ...
    def set_eigenvalues(self, min_eig, max_eig, matrix, rng):
        '''Sets eigenvalues of matrix.'''
        U_inv = jnp.linalg.inv(matrix)
        gen_rng, shuffle_rng = random.split(rng)
        diag = jnp.diag(random.permutation(shuffle_rng,jnp.concatenate([random.uniform(gen_rng, shape=[matrix.shape[-1]-1],
                            minval=min_eig, maxval=max_eig), jnp.array([max_eig])])))
        W = matrix@diag@U_inv
        return W

    def maped_scale(self, W, rngs, min_eig, max_eig):
        '''Vectorizes eigenvalue-scaling for a batch of matrices.'''
        scale_fn = partial(self.set_eigenvalues, min_eig=min_eig, max_eig=max_eig)
        return vmap(scale_fn)(matrix=W, rng=rngs)
    ...
    def create_batch(self, 
        ...
        '''
        rng, subkeyW, subkeyX, subkeyN1, subkeyN2, subkeyObs, subkeyScales = random.split(rng, 7)
        scale_key_batch = random.split(subkeyScales, batch_size)
        batch_of_noise_keys = random.split(subkeyN1, batch_size)
        W = random.normal(key=subkeyW, 
                          shape=(batch_size, data_dim, data_dim))
        W = self.maped_scale(W=W, 
                             rngs=scale_key_batch, 
